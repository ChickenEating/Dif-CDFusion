...
class Evaluator:
    ...
    @classmethod
    def VIF(cls, reference, distorted):
        """
        计算标准VIF（Visual Information Fidelity）指标。
        
        参数：
        - reference: 原始图像（参考图像）
        - distorted: 失真图像（被评估的图像）
        
        返回：
        - VIF指标
        """
        cls.input_check(reference, distorted)
        ...
        filter_size = 5
        filter_sigma = 1.0
        kernel = gaussian_kernel(filter_size, filter_sigma)

        # 图像尺寸
        height, width = reference.shape

        # 计算局部均值和方差
        def local_statistics(image, kernel):
            mean = convolve(image, kernel, mode='constant', cval=0)
            variance = convolve(image ** 2, kernel, mode='constant', cval=0) - mean ** 2
            return mean, variance

        # 计算参考图像的均值和方差
        mu_ref, sigma_ref_sq = local_statistics(reference, kernel)
        # 计算失真图像的均值和方差
        mu_dist, sigma_dist_sq = local_statistics(distorted, kernel)

        # 计算局部协方差
        sigma_ref_dist = convolve(reference * distorted, kernel, mode='constant', cval=0) - mu_ref * mu_dist

        # 计算g和sv
        g = sigma_ref_dist / (sigma_ref_sq + eps)
        sv = sigma_dist_sq - g * sigma_ref_dist

        # 防止负方差
        g[sigma_ref_sq < eps] = 0
        sv[sigma_ref_sq < eps] = sigma_dist_sq[sigma_ref_sq < eps]

        # VIF计算公式
        num = np.sum(np.log(1 + g ** 2 * sigma_ref_sq / (sv + sigma_nsq)))
        den = np.sum(np.log(1 + sigma_ref_sq / sigma_nsq))

        # 返回VIF指标
        vif_value = num / (den + eps)

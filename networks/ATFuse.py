...
class ATF(nn.Module):
    def __init__(self, opt):
        ...
        if self.use_aggregate:
            ...
            self.resBlock = ResBlock(in_channels=embed_dims[0] * num_paths[1], out_channels=embed_dims[0] * num_paths[1])
            self.up_scale2_aggregate = UpScale(opt["networks"]["feature_sum"], embed_dim=embed_dims[0] * num_paths[1])
            # self.head = Fusion_Head()
            self.head = ResBlock(embed_dims[0] // 2, embed_dims[0] // 2)
            # # self.head = nn.ModuleList(*[ResBlock(embed_dims[0] // 2, embed_dims[0] // 2) for _ in range(num_paths[2])])
            self.head_middle = ResBlock(embed_dims[0] // 2, embed_dims[0] // 4)
            self.head_final = ResBlock(embed_dims[0] // 4, out_chans)
            # # self.head_final = nn.ModuleList([ResBlock(embed_dims[0] // 4, out_chans) for _ in range(num_paths[3])])
        else:
            # upscale 部分
    ...
    def forward(self, vi, ir):
        ...
        with torch.no_grad():
            ...
            # ir,vi.shape = [X, X, 480, 640]

        # # Shallow Feature Extraction 我要使用diffusion模型进行特征提取
        # att_outputs = []
        # for x, model in zip(inputs, self.stem1):
        #     att_outputs.append(model(x))
        # # att_outputs这个list有两个CHW=64*64*64的tensor，C=64是因为json文件中embed_dims[0]=64
        # # 我的diffusion模型提取的特征的格式要和att_outputs一样
        
        # with torch.no_grad():
        #     df_feats = self.diffusion_feature(vi, ir) # df_feats.shape == 6, 128, 128, 128
        
        # # Patch Embedding
        att_outputs = self.patch_embed_stages1(inputs)
        ir, vi = att_outputs[0], att_outputs[1]

        # 下面是Feature Fusion模块的三个交叉注意力机制
        att_outputs2 = []  # mhca_stage是DIIM
        att_outputs2.append(self.mhca_stage(att_outputs[1], ir, ir, minus=False))
        self.difference = att_outputs2[0]
        
        att_outputs2_1 = []  # mhca_stage1_2是ACIIM
        att_outputs2_1.append(self.mhca_stage1_2(att_outputs2[0], vi, vi, minus=True))

        att_outputs3 = []  # mhca_stage2是ACIIM
        att_outputs3.append(self.mhca_stage2(att_outputs2_1[0], ir, ir, minus=True))
        self.common = att_outputs3[0]
        ...
        if self.use_aggregate:
            ...
            # x22.shape:  torch.Size([22, 64, 256, 256])
            x = self.head(x22)
            # x = self.head(x)
            # x = self.head(x)
            x = self.head(x)
            x = self.downsample(x)
            # self.head(x22).shape:  torch.Size([22, 32, 256, 256])
            x = self.head_middle(x)
            x = self.head_final(x)
            # # self.head_final(x).shape:  torch.Size([22, 3, 256, 256])
            # x = self.head(x22)
            # self.downsample(x).shape:  torch.Size([22, 3, 128, 128])
